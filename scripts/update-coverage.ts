 ...
 *
 * Reads the de-energy.db, queries each table for row counts,
...
const DB_PATH = process.env["DE_ENERGY_DB_PATH"] ?? join(__dirname, "..", "data", "de-energy.db");
...
const SOURCE_QUERIES: Record<string, string> = {
  bnetza: "SELECT COUNT(*) as c FROM regulations WHERE regulator_id = 'bnetza'",
  bfe: "SELECT COUNT(*) as c FROM regulations WHERE regulator_id = 'bfe'",
  tso: "SELECT COUNT(*) as c FROM grid_codes",
  bmwk: "SELECT COUNT(*) as c FROM decisions",
};
